"""
asp_mn40_v4.py  —  ASP on ModelNet40, targeting > 92% OA
=========================================================
Colab:  !python asp_mn40_v4.py   (checkpoints → Google Drive)
Kaggle: add dataset balraj98/modelnet40-princeton-3d-object-dataset
...
# trimesh and kagglehub needed on both platforms (kagglehub pre-installed on Kaggle)
...
DROP_PATH   = 0.3
ASP_STEPS   = 4
APTEC_T     = 4        # pseudo-timesteps in MaxFirst-APTEC encoder (paper: T=4)
K_GROUP     = 16       # inter-group KNN for GroupMaxFirstEdgeConv (spatial graph)
...
# Paths — Kaggle uses /kaggle/working for outputs; Colab uses Drive
if ON_KAGGLE:
    CKPT_DIR     = "/kaggle/working/asp_mn40_v4_ckpts"
    # /kaggle/input/ is read-only; downloaded data must go to /kaggle/working/
    _MN40_INPUT  = "/kaggle/input/modelnet40-princeton-3d-object-dataset"
    _MN40_WORK   = "/kaggle/working/ModelNet40"
    MN40_DIR     = _MN40_INPUT  # will be resolved in section 3
else:
    CKPT_DIR = "/content/drive/MyDrive/asp_mn40_v4_ckpts"
...
ASP_LATEST     = os.path.join(CKPT_DIR, "asp_mn40_latest.pt")
ASP_BEST       = os.path.join(CKPT_DIR, "asp_mn40_best.pth")
...
print(f"MN40 data  : {MN40_DIR}")

# ─── 3. Download / locate ModelNet40 ─────────────────────────────────────────
import shutil, glob as _glob
import kagglehub

_MN40_DATASET = "balraj98/modelnet40-princeton-3d-object-dataset"


def _find_mn40(base: str) -> str | None:
    """Return path to the ModelNet40 class-folder tree, or None."""
    for candidate in [os.path.join(base, "ModelNet40"), base]:
...
def _download_mn40(dest: str):
    print(f"\nDownloading ModelNet40 via kagglehub → {dest} …")
    p = kagglehub.dataset_download(_MN40_DATASET)
    found = _find_mn40(p)
    if found:
        if found != dest:
...
if ON_KAGGLE:
    # Priority 1: pre-mounted dataset input (read-only, but instant)
    pre = _find_mn40(_MN40_INPUT)
    if pre:
        MN40_DIR = pre
        print(f"[Kaggle] MN40 pre-mounted at {MN40_DIR}")
    else:
        # Priority 2: already downloaded to working dir in a prior run
        pre2 = _find_mn40(_MN40_WORK)
        if pre2:
            MN40_DIR = pre2
            print(f"[Kaggle] MN40 cached at {MN40_DIR}")
        else:
            # Priority 3: download via kagglehub into writable /kaggle/working/
            MN40_DIR = _MN40_WORK
...
#
# Problem with OfficialLikeEncoder:
#   sp1 fires → then gl = x.max(3, keepdim=True).values
#   Binarising before max collapses {0.1, 0.9} to {0, 1}: max is now logical-OR,
#   losing the geometric magnitude ordering that tells EdgeConv which neighbour
#   had the STRONGEST response.
#
# Fix (Max-First Rule): all Conv2d and max-pooling run in continuous domain;
# the single APTECNeuron fires AFTER both max aggregations.
#
# APTEC adds adaptive threshold V_th = 1 + 0.5·sigmoid(x) ∈ (1, 1.5):
#   - suppresses borderline spurious spikes (denominator > 1 tightens the gate)
#   - pulls saturated units (u_hat > V_th, i.e. z > 1) back into the
#     surrogate-gradient-active region [0,1], restoring trainability.
...
class APTECNeuron(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for _ in range(self.T):
            u     = self.decay * u + x              # leaky integrate
            u_hat = _mpr(u)                         # MPR clamp
            v_th  = 1.0 + 0.5 * torch.sigmoid(x)   # adaptive threshold ∈ (1, 1.5)
            z     = u_hat / v_th                    # normalised membrane variable
            s     = spike_fn(z - 0.5)              # fire when z > 0.5
            u     = u - s                           # soft reset
            spikes.append(s)
        return torch.stack(spikes, dim=0).max(dim=0).values   # temporal-OR


class MaxFirstAPTECEncoder(nn.Module):
    """
    Drop-in replacement for OfficialLikeEncoder with Max-First Spiking Rule.

    All Conv2d and both max-pool operations execute in continuous (float) domain.
    A single APTECNeuron binarises the group-level features AFTER max aggregation.
    This preserves geometric magnitude ordering during neighbourhood aggregation.
    """
    ...
    def forward(self, nh: torch.Tensor) -> torch.Tensor:
        ...
        # Continuous-domain feature extraction — no spikes before max
...
class GroupMaxFirstEdgeConv(nn.Module):
    ...
    """
    def __init__(self, dim: int, k_group: int = 16, T: int = 4, decay: float = 0.9):
        super().__init__()
        self.k = k_group
