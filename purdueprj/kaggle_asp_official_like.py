...
pkgs = ['trimesh', 'reportlab', 'kagglehub']
for p in pkgs:
...
# â”€â”€ Patch: fix in-place visited_mask update (causes autograd RuntimeError) â”€â”€â”€â”€
...
_new = (
    "            selected_idx = scores.masked_fill(visited_mask, float(\"-inf\")).argmax(dim=-1)\n"
    "            # non-in-place update â€” avoids corrupting autograd saved tensors\n"
...
DATASET_DIR = '/kaggle/working/ModelNet10'
...
if not os.path.isdir(DATASET_DIR):
    ...
    # Locate ModelNet10 subfolder
    mn10_src = None
    for root, dirs, _ in os.walk(p):
        if 'ModelNet10' in dirs:
            mn10_src = os.path.join(root, 'ModelNet10')
            break
    if mn10_src:
        shutil.copytree(mn10_src, DATASET_DIR)
        print(f'  Copied: {mn10_src} → {DATASET_DIR}')
    else:
        # Try zip extraction fallback
        zips = glob.glob(os.path.join(p, '*.zip'))
        if zips:
            os.system(f'unzip -q "{zips[0]}" -d /kaggle/working/')
        else:
            raise RuntimeError(f'ModelNet10 not found in Kaggle download at {p}')

    if not os.path.isdir(DATASET_DIR):
        raise RuntimeError(f'{DATASET_DIR} still missing after download.')
    print('Done.')
else:
...
MN40_DIR = '/kaggle/working/ModelNet40'
...
if not os.path.isdir(MN40_DIR):
    ...
    if mn40_src:
        ...
        if zips:
            ...
            raise RuntimeError(f'ModelNet40 not found in download at {p}')
...
import sys
sys.path.insert(0, '/kaggle/working/asp')
...
train_ds = ModelNetDataset(root='/kaggle/working/ModelNet10', split='train', num_points=1024)
val_ds   = ModelNetDataset(root='/kaggle/working/ModelNet10', split='test',  num_points=1024)
...
NUM_SLICES = 16
THRESHOLD  = 0.7
SAVE_DIR   = '/kaggle/working/asp_results'
...
# ── Config ────────────────────────────────────────────────────────────────────
EPOCHS     = 30          # change to 150 for full SOTA run
NUM_SLICES = 16
THRESHOLD  = 0.7
SAVE_DIR   = '/kaggle/working/asp_results_mn40'
os.makedirs(SAVE_DIR, exist_ok=True)
...
MN40_DIR = '/kaggle/working/ModelNet40'
if not os.path.isdir(MN40_DIR):
    raise RuntimeError('ModelNet40 not found -- run cell 3b first.')

train_ds40 = ModelNetDataset(root=MN40_DIR, split='train', num_points=1024)
val_ds40   = ModelNetDataset(root=MN40_DIR, split='test',  num_points=1024)
train_loader40 = DataLoader(train_ds40, batch_size=16, shuffle=True,
                            num_workers=2, pin_memory=True)
val_loader40   = DataLoader(val_ds40,   batch_size=16, shuffle=False,
                            num_workers=2, pin_memory=True)
print(f'MN40 -- Train: {len(train_ds40)}  Val: {len(val_ds40)}')
...
params = model40.param_count()
print(f'Total params: {params["total"]:,}')

optimizer40 = build_optimizer(model40, lr=1e-3, weight_decay=1e-4)
scheduler40 = torch.optim.lr_scheduler.CosineAnnealingLR(
    optimizer40, T_max=EPOCHS, eta_min=1e-5)

history40   = []
best_acc40  = 0.0
best_ckpt40 = os.path.join(SAVE_DIR, 'best_model_mn40.pth')

for epoch in range(EPOCHS):
    print(f'\n--- Epoch {epoch}/{EPOCHS-1} ---')
    train_m = train_active_epoch(
        model40, train_loader40, optimizer40, device, epoch=epoch,
        num_slices=NUM_SLICES, verbose_every=20)
    scheduler40.step()

    val_m = {}
    if (epoch + 1) % 5 == 0 or epoch == EPOCHS - 1:
        val_m = validate_active(model40, val_loader40, device,
                                num_slices=NUM_SLICES, threshold=THRESHOLD)
        val_acc = val_m['acc']
        print(f'[Val] Acc={val_acc:.4f}  '
              f'MeanExit={val_m["mean_exit"]:.2f}/{NUM_SLICES}  '
              ...
              f'EnergyRatio={val_m["energy_ratio"]:.4f}  '
              f'Savings={val_m["savings"]:.1f}x')
        if val_acc > best_acc40:
            best_acc40 = val_acc
            torch.save({'epoch': epoch, 'model_state': model40.state_dict(),
                        'val_acc': val_acc}, best_ckpt40)
            print(f'  *** New best MN40: {val_acc:.4f} -> saved')

    history40.append({'epoch': epoch, **train_m, **val_m})
    with open(os.path.join(SAVE_DIR, 'history_mn40.json'), 'w') as f:
        json.dump(history40, f, indent=2)

print(f'\nMN40 Best Val Acc: {best_acc40:.4f}')
print(f'Checkpoint: {best_ckpt40}')
print('\n--- Verdict ---')
if best_acc40 >= 0.85:
    print('  Looks promising! Consider scaling to 150 epochs on A100/H100.')
elif best_acc40 >= 0.75:
    print('  Moderate results -- try 150 epochs or tune lr/num_slices.')
else:
    print('  Low accuracy so far -- check dataset path and model config.')
...
device = 'cuda' if torch.cuda.is_available() else 'cpu'
ckpt   = torch.load('/kaggle/working/asp_results/best_model.pth', map_location=device)
...
with open('/kaggle/working/asp_results/pareto_curve.json', 'w') as f:
...
os.makedirs('/kaggle/working/asp_results/figs', exist_ok=True)

# Fig 1: Pareto frontier
with open('/kaggle/working/asp_results/pareto_curve.json') as f:
    curve = json.load(f)
...
PA.plot_pareto(curve, fixed_baselines, save_path='/kaggle/working/asp_results/figs/fig1_pareto.png')
...
with torch.no_grad():
    for pts, labels in val_loader:
        ...
        if len(exit_steps) >= 200:
            break
PA.plot_exit_distribution(exit_steps, save_path='/kaggle/working/asp_results/figs/fig2_exit.png')

# Fig 3: Training history
with open('/kaggle/working/asp_results/history.json') as f:
    history = json.load(f)
PA.plot_training_history(history, save_path='/kaggle/working/asp_results/figs/fig3_history.png')

# Display all figures inline
for figpath in ['fig1_pareto', 'fig2_exit', 'fig3_history']:
    path = f'/kaggle/working/asp_results/figs/{figpath}.png'
...
result = subprocess.run(
    ['python', '/kaggle/working/asp/generate_pdf.py'],
    capture_output=True, text=True, cwd='/kaggle/working/asp'
)
print(result.stdout or 'generate_pdf.py not found in /kaggle/working/asp')
if result.stderr:
    print('STDERR:', result.stderr[:500])


# Results are saved to /kaggle/working/asp_results/
# Kaggle automatically makes /kaggle/working/ available as notebook output.
# After the notebook finishes, click "Data" in the right panel → "Output"
# to download files, or use the three-dot menu on the notebook to save a version.

import os, shutil

out_dir = '/kaggle/working/asp_results'
zip_path = '/kaggle/working/asp_results_export.zip'

if os.path.isdir(out_dir):
    shutil.make_archive('/kaggle/working/asp_results_export', 'zip', out_dir)
    print(f'Zipped results → {zip_path}')
    print('Download it from the Kaggle Output tab on the right.')
else:
    print('No results directory found yet — run training first.')


# Resume Training from Checkpoint
# Upload best_model.pth to /kaggle/working/ via the Kaggle dataset input,
