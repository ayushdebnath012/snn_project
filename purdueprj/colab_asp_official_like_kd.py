"""
colab_asp_official_like_kd.py
=============================
Colab/T4 version of kaggle_asp_official_like_kd.py.

Recommended runtime:
  Runtime -> Change runtime type -> T4 GPU

Run from a Colab notebook cell:
  !python colab_asp_official_like_kd.py

Useful quick smoke run:
  !EPOCHS=1 TEACHER_EPOCHS=1 DATASETS=ModelNet10 python colab_asp_official_like_kd.py

Default Colab paths:
  local work/data/results: /content/asp_official_like_kd
  optional Drive mirror:  /content/drive/MyDrive/asp_official_like_kd_checkpoints

If Kaggle credentials are required for kagglehub, set Colab Secrets:
  KAGGLE_USERNAME
  KAGGLE_KEY
...
Outputs:
  /content/asp_official_like_kd/official_like_ckpts/final_results.json
  /content/asp_official_like_kd/official_like_ckpts/01_training_curves.png
  /content/asp_official_like_kd/official_like_ckpts/02_accuracy_bars.png
  /content/asp_official_like_kd/official_like_ckpts/debug_*_minimal.pt
  /content/asp_official_like_kd/official_like_ckpts/debug_runtime_summary.json
  /content/asp_official_like_kd/official_like_ckpts/spm_<dataset>_best.pth
  /content/asp_official_like_kd/official_like_ckpts/asp_<dataset>_best.pth
"""
...
ON_COLAB = bool(os.environ.get("COLAB_RELEASE_TAG")) or os.path.isdir("/content")
...
if torch.cuda.is_available():
    ...
    print("WARNING: No GPU detected. In Colab use Runtime -> Change runtime type -> T4 GPU")

for pkg in ["trimesh", "matplotlib"]:
    subprocess.run(
...
WORK = os.environ.get(
    "ASP_COLAB_ROOT",
    "/content/asp_official_like_kd" if ON_COLAB else os.path.abspath("asp_official_like_kd"),
)
...
def mount_drive_if_requested():
    if not env_bool("MOUNT_DRIVE", ON_COLAB):
        print("[DriveSync] MOUNT_DRIVE=0 - checkpoints are local only.")
        return ""

    drive_ckpt_dir = os.environ.get(
        "DRIVE_CKPT_DIR",
        "/content/drive/MyDrive/asp_official_like_kd_checkpoints",
    )

    if not ON_COLAB:
        print("[DriveSync] Not running in Colab - Drive sync disabled.")
        return ""
    ...
    try:
        ...
        print(f"[DriveSync] Checkpoints mirrored to {drive_ckpt_dir}")
        return drive_ckpt_dir
    except Exception as exc:
        print(f"[DriveSync] Drive mount failed: {exc}. Checkpoints are local only.")
        return ""


DRIVE_CKPT_DIR = mount_drive_if_requested()
...
if ON_GPU:
    ...
    print("WARNING: CPU demo mode. Use a Colab T4 GPU runtime for the real run.")
...
class DriveSync:
    """
    Mirror local checkpoint files into a mounted Colab Drive folder.

    This keeps training fast by using /content for active I/O, while still
    restoring from Drive after a runtime disconnect.
    """

    def __init__(self, folder):
        self._folder = folder
        self._enabled = bool(folder)
    ...
    def upload(self, local_path):
        """Copy local_path into the Drive mirror folder."""
        if not self.enabled or not os.path.isfile(local_path):
            return
        try:
            os.makedirs(self._folder, exist_ok=True)
            shutil.copy2(local_path, os.path.join(self._folder, os.path.basename(local_path)))
        except Exception as exc:
            print(f"  [DriveSync] mirror failed for {local_path}: {exc}")

    def download(self, filename, local_path):
        """Restore filename from the Drive mirror if local_path is missing."""
        if not self.enabled or os.path.isfile(local_path):
            return
        src = os.path.join(self._folder, filename)
        if not os.path.isfile(src):
            return
        try:
            os.makedirs(os.path.dirname(local_path), exist_ok=True)
            shutil.copy2(src, local_path)
            print(f"  [DriveSync] restored {filename} from Drive")
        except Exception as exc:
            print(f"  [DriveSync] restore failed for {filename}: {exc}")


drive_sync = DriveSync(DRIVE_CKPT_DIR)
...
def _download(name, url):
    ...
    for d in os.listdir(DATA_DIR):
        ...
        if os.path.isdir(candidate) and d != name:
            has_train = any(
                os.path.isdir(os.path.join(candidate, c, "train"))
                for c in os.listdir(candidate)
            )
            if has_train:
                shutil.move(candidate, folder)
                print(f"  {name} -> {folder}")
                return folder

    print(f"  {name}: using DATA_DIR {DATA_DIR}")
    return DATA_DIR
...
def train_teacher_epoch(model, loader, opt, max_steps=None):
    ...
    for step, (pts, labels) in enumerate(loader):
        ...
        if hit_step_cap:
            break
    return total_loss / max(1, n), total_acc / max(1, n), _timed_stats(t_start, steps_done, n)
...
def prepare_teacher(ds_name, num_classes, train_loader, val_loader):
    ...
    for ep in range(start_ep, TEACHER_EPOCHS):
        ...
        teacher_hist.append({"ep": ep + 1, "tr": tr_acc, "val": val_acc, "timing": train_stats})
        save_ckpt(
            latest, teacher, opt, sch, ep + 1, best_teacher, teacher_hist,
            debug_stats=train_stats if DEBUG_MINI_RUN else None,
        )
        save_debug_ckpt(
            "teacher", ds_name, teacher, opt, sch, ep + 1, best_teacher, teacher_hist, train_stats
        )
...
def dataset_config():
    ...
    if "ModelNet10" in DATASET_NAMES:
        if MN10_DIR is None:
            ...
            MN10_DIR = _download("ModelNet10", DATASET_URLS["ModelNet10"])
        cfg["ModelNet10"] = {"root": MN10_DIR, "classes": 10}
    if "ModelNet40" in DATASET_NAMES:
        if MN40_DIR is None:
            print("\n[1] Downloading ModelNet40 ...")
            MN40_DIR = _download("ModelNet40", DATASET_URLS["ModelNet40"])
...
def main():
    ...
    for ds_name, ds_cfg in dataset_config().items():
        ...
        for ep in range(start_ep, EPOCHS):
            ...
            spm_hist.append({"ep": ep + 1, "tr": tr_acc, "val": val_acc, "timing": train_stats})
            save_ckpt(
                spm_latest, spm, spm_opt, spm_sch, ep + 1, best_spm, spm_hist,
                debug_stats=train_stats if DEBUG_MINI_RUN else None,
            )
            save_debug_ckpt(
                "spm", ds_name, spm, spm_opt, spm_sch, ep + 1, best_spm, spm_hist, train_stats
            )
        ...
        for ep in range(start_ep, EPOCHS):
            ...
            asp_hist.append({"ep": ep + 1, "tr": tr_acc, "val": val_acc, "timing": train_stats})
            save_ckpt(
                asp_latest, asp, asp_opt, asp_sch, ep + 1, best_asp, asp_hist,
                debug_stats=train_stats if DEBUG_MINI_RUN else None,
            )
            save_debug_ckpt(
                "asp", ds_name, asp, asp_opt, asp_sch, ep + 1, best_asp, asp_hist, train_stats
            )
    ...
    # Zip outputs for easy download from the Colab file browser.
    import shutil as _shutil

    zip_base = os.path.join(WORK, "asp_kd_results_export")
    zip_file = _shutil.make_archive(zip_base, "zip", CKPT_DIR)
    drive_sync.upload(zip_file)
    print(f"Zipped results -> {zip_file}")
    if drive_sync.enabled:
        print(f"Drive mirror    -> {DRIVE_CKPT_DIR}")
    if ON_COLAB and env_bool("COLAB_AUTO_DOWNLOAD_ZIP", False):
        try:
            from google.colab import files

            files.download(zip_file)
        except Exception as exc:
            print(f"Colab download prompt failed: {exc}")
    print("Download the zip from the Colab file browser, or set COLAB_AUTO_DOWNLOAD_ZIP=1.")
