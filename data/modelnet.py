...
def augment_point_cloud(pts, mode="baseline", rng=np.random, normalize_after=False):
    ...
    if mode in ("baseline", "standard"):
        ...
        pts = pts @ _rotation_matrix("z", angle).T
        if rng.random() < 0.35:
            pts = pts @ _rotation_matrix("x", rng.uniform(-0.12, 0.12)).T
            pts = pts @ _rotation_matrix("y", rng.uniform(-0.12, 0.12)).T
        ...
        pts = pts @ _rotation_matrix("z", rng.uniform(0.0, 2.0 * np.pi)).T
        ...
        for class_name in self.class_names:
            ...
            for f in os.listdir(class_path):
                if f.endswith((".npy", ".txt", ".off")):
                    items.append((os.path.join(class_path, f), label_map[class_name]))
        return items
        ...
        for path, label in self.files:
            ...
            if not path.endswith(".off"):
                if pts.shape[0] >= self.num_points:
                    ...
                    rep = np.random.choice(pts.shape[0], self.num_points - pts.shape[0], replace=True)
